  ...
  <section className="test-page">
    <div className="test-page__hero">
      <div>
        ...
        <p>
          {isTeacher
            ? "Create tests and review each test's results."
            : "Multiple-choice tests are ready when your teacher publishes them."}
        </p>
      </div>

      {isTeacher && (
        <button
          className="test-action test-action--primary"
          onClick={() => navigate(`/myclass/${courseId}/add-test`)}
        >
      ...
      <>
        <div className="test-list-toolbar">
          <SectionHeading
            icon={faBookOpen}
            title="Created Tests"
            text="Select a test to view only that test's student results."
          />
        ...
        <div className="test-grid">
          {teacherTests.map((test) => {
            const testSubmissions = submissions.filter(
              (item) => item.test === test.id
            );

            const average = testSubmissions.length
              ? (
            ...
            return (
              <article
                className="test-card test-card--teacher"
                key={test.id}
              >
                ...
                <div className="test-metrics">
                  ...
                  <span>
                    <strong>{enrollments.length}</strong> Students
                  </span>

                  <span>
                    <strong>{testSubmissions.length}</strong> Submitted
                  </span>
                ...
                <div className="test-card__actions">
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/myclass/${courseId}/tests/${test.id}/results`
                      )
                    }
                  >
                  ...
                  {test.published ? (
                    <button
                      className="test-action test-action--secondary"
                      disabled={busyId === test.id}
                      onClick={() =>
                        updatePublication(test, false)
                      }
                    >
                      Unpublish
                    </button>
                    ...
                    <>
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `/myclass/${courseId}/tests/${test.id}/edit`
                          )
                        }
                      >
                        <FontAwesomeIcon
                  ...
                  <button
                    className="test-icon-button test-icon-button--danger"
        ...
        {!teacherTests.length && (
          <EmptyState text="No tests created yet. Create a test to get started." />
        )}
      </>
    ) : (
      <>
        <SectionHeading
          ...
          text="Each test has one attempt and four choices per question."
        />

        <div className="test-grid">
          {availableTests.map((test) => studentCard(test))}
        </div>

        {!availableTests.length && (
          <EmptyState text="There are no tests available right now. Check back after your teacher publishes one." />
        )}

        <SectionHeading
          icon={faCheckCircle}
          title="Completed Tests"
          text="Your submitted assessments."
        />

        <div className="test-grid">
          {completedTests.map((test) =>
            studentCard(test, true)
          )}
        </div>

        {!completedTests.length && (
          <EmptyState text="You have not completed a test yet." />
        )}
      </>
    )}
  </section>
